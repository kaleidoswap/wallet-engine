...
import { mapRgbStatus, rgbBtcAsset, rgbNiaAsset, rgbAssetBalance } from '../src/adapters/wdk/RgbCore'
...
describe('RgbCore translation helpers', () => {
  ...
  it('builds the BTC asset entry', () => {
    const a = rgbBtcAsset(12345)
    expect(a).toMatchObject({ id: 'BTC', ticker: 'BTC', protocol: 'RGB', layer: 'BTC_L1' })
    expect(a.balance.total).toBe(12345)
  })

  it('maps a NIA asset with the default RGB-LN profile', () => {
    const a = rgbNiaAsset({ asset_id: 'rgb:USDT', ticker: 'USDT', name: 'Tether', precision: 2, balance: { spendable: 500 } })
    expect(a).toMatchObject({ id: 'rgb:USDT', ticker: 'USDT', name: 'Tether', precision: 2, layer: 'RGB_LN' })
    expect(a.balance.total).toBe(500)
    expect(a.capabilities.supportsLightning).toBe(true)
  })

  it('applies RGB-L1 overrides (no lightning)', () => {
    const a = rgbNiaAsset(
      { asset_id: 'rgb:XAUT', balance: { settled: 3 } },
      { layer: 'RGB_L1', capabilities: { supportsLightning: false } }
    )
